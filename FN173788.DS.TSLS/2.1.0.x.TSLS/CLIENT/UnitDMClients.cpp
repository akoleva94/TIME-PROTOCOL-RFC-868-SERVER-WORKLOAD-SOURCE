...
void __fastcall tsp::ClientsPack::Save(void)
{
	...
	if(file)
	{
		...
		file << "Режим на протокола:;;;;" << Parameters.lmMode.ToStr(false) << std::endl;
...
//---------------------------------------------------------------------------
void __fastcall tsp::Log::Add(String str, Parameters::LogMode lm)
{
	if(lm != lm.No)
	{
...
//---------------------------------------------------------------------------
void __fastcall tsp::Log::Add(TCustomWinSocket* sock, String str, Parameters::LogMode lm)
{
	if(lm != lm.No)
	{
		String ws = FormatDateTime("hh:mm:ss.zzz",Time()) +
...
//---------------------------------------------------------------------------
void __fastcall tsp::Log::Show(TMemo* dst, Parameters::LogMode lm)
{
	for(String i : Lines)
	{
		dst->Lines->Add(i);
...
void __fastcall TdmClients::__read_ini_file(tsp::Parameters& dst)
{
	...
	// Секция Debug
	dst.lmMode	= iniFile->ReadInteger(L"Debug", L"LOG_MODE", 0);
...
void __fastcall TdmClients::SendBatchOfReq(void)
{
	...
	if(clientsPack->Parameters.boolConsecutive)
	{
		if(clientsPack->vClients.size() > 0)
		{
			...
			String str = "SYN [" + clientsPack->Parameters.lmMode.ToStr() + "]";
			AddToLog(str, clientsPack->dtTsyn);
//			log->Add("SYN", Parameters.lmMode);
		}
	}
	else
	{
...
void __fastcall TdmClients::csOutConnect(TObject *Sender, TCustomWinSocket *Socket)
{
	...
	log->Add(Socket, "CNC[" + IntToStr(intReqId) + "]", Parameters.lmMode);
...
{
	int intReqId = GetChannelId(Sender);
	log->Add(Socket, "DSC[" + IntToStr(intReqId) + "]", Parameters.lmMode);
	...
	if(clientsPack->intPending == 0)
	{
		...
		log->Show(formMain->memoLog, Parameters.lmMode);

		String str = "END [" + clientsPack->Parameters.lmMode.ToStr() + "]";
		AddToLog(str, clientsPack->dtTend);
...
{
	int intReqId = GetChannelId(Sender);
	log->Add(Socket, "ERR[" + IntToStr(intReqId) + "][" + IntToStr(ErrorCode) + "]", Parameters.lmMode);
...
/////////////////////////////////////////////////////////////////////////////
	...
	std::thread threadWorking(DoWork, Socket, intReqId, this);
#else
	std::thread threadWorking = std::thread([Socket, intReqId, this]()
		{
			unsigned long ulTime;
			...
			this->log->Add(Socket, "RPL[" + IntToStr(intReqId) + "]::[" + IntToHex((int)ulTime, 8) + "]", this->Parameters.lmMode);

			std::chrono::milliseconds sleep_dur(this->Parameters.intDelayToFree);

...
//---------------------------------------------------------------------------
void DoWork(Item c, int id, TObject* o)
{
//	TdmClients* oo = (TdmClients*)o;
	...
	oo->log->Add(c, "RPL[" + IntToStr(id) + "]::[" + IntToHex((int)ulTime, 8) + "]", oo->Parameters.lmMode);

	std::chrono::milliseconds sleep_dur(oo->Parameters.intDelayToFree);

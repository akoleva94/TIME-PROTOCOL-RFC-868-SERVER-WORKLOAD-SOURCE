...
__fastcall TformSockets::TformSockets(TComponent* Owner)
	: TForm(Owner)
{
	intWorkingThreadsNumber = 0;
    intWorkingThreadsNumberMax = 0;
}
//---------------------------------------------------------------------------
// КЛИЕНТСКА ЧАСТ ///////////////////////////////////////////////////////////
//---------------------------------------------------------------------------
...
/* TODO : OnConnect */
	...
	// ЗАДЪРЖАНЕТО В ТАЗИ ТОЧКА ВОДИ ДО ОТХВЪРЛЯНЕТО НА ОПИТИТЕ ЗА СВЪРЗВАНЕ,
	// ФОРМИРАНИ ЕДНОВРЕМЕННО

	// Sleep(1);    // за демонстриране на този ефект

	// Затова цялата обработка тук е изнесена в обслужваща нишка

	std::thread threadWorking(DoWork, Socket);	// създаване/стартиране на обслужваща нишка

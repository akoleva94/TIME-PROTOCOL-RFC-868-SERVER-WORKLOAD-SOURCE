...
std::mutex mutex;
//---------------------------------------------------------------------------
void __fastcall tsp::Log::Clear(void)
{
	String ws = FormatDateTime("hh:mm:ss.zzz",Time()) + " SYN";
...
{
	String ws = FormatDateTime("hh:mm:ss.zzz",Time()) + " READY";
	formMain->memoLog->Lines->Add(ws);
...
void __fastcall TdmClients::SendBatchOfReq(void)
{
	...
	clientsPack.intPending = intVolume;
